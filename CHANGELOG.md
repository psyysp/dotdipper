...
- **`scripts/audit-apps.sh` — what the repository expects versus what the machine has.** After a bootstrap run where some packages were declined and others failed, there was no way to see the difference. It splits casks three ways rather than two, because "brew did not install it" and "it is not installed" are different facts and only one needs action: on the author's machine that is 21 via brew, 29 present but installed by hand, and 1 genuinely absent — a two-way check would have reported 30 false problems. It also says when App Store titles went unchecked because `mas` is missing or signed out, rather than counting them as absent, and flags application bundles that are symlinks to nowhere, which look installed in Finder and satisfy nothing.

### Fixed

- **An unattended cask install reported success for driver extensions that do nothing.** `brew install --cask` finishes once the app is on disk, but macOS will not load a driver extension until a person approves it — and Karabiner does not even submit its DriverKit extension until the app or its `Karabiner-VirtualHIDDevice-Manager` helper has run once. A machine bootstrapped without a human present therefore ends up with the app installed, the extension inert, and every script in the chain reporting success. The generated install script now closes with a check that reports both states: any extension macOS knows about but has not enabled, found by state so it covers every vendor, and Karabiner's never-submitted case, which is invisible to `systemextensionsctl` and is found by the helper's presence instead. It names `System Settings > General > Login Items & Extensions`, which is where the control lives on current macOS rather than the pre-Ventura Privacy & Security pane. The stage reports and never activates: activation raises a GUI dialog and opens a `file://` URL, neither of which belongs in an unattended run.
...
- **A profile overlay could silently override the global config, and nothing said so.** `profiles/<name>/config.toml` is merged over `config.toml` with overlay keys winning, and discovery writes `tracked_files` and `packages.common` to the overlay — so the global file kept a stale copy of both that still read as authoritative. On the author's machine the global config listed 49 tracked paths against the overlay's 74, and hand-editing the global list changed nothing: the snapshot kept capturing the old set, with no warning, no diff, and no indication which file was in charge.
  ...
  - `config --edit` lists every shadowed key before opening the editor, and `config --edit --overlay` opens the overlay directly. `config --show` and `doctor` report the same list.
  - Discovery removes the global copy of a key once the overlay owns it, so the two files cannot drift apart again — unless another profile has no overlay value of its own and would still inherit that copy, in which case it stays. Healing a shadowed key must not delete a live value out from under a profile that never asked. One writable owner per key is now the invariant, and `doctor` fails when it is violated.
